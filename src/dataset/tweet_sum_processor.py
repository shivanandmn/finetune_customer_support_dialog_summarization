...
class Turn:
    ...
    def get_json(self) -> json:
        result = {"is_agent": self.is_agent(), "sentences": self.sentences}
        return json.dumps(result)

    def __str__(self):
        return ("Agent:\t" if self.is_agent() else "Customer:\t") + " ".join(
            self.sentences
        )
...
class Dialog:
    ...
    def get_json(self) -> json:
        ...
        result = {"dialog_id": self.dialog_id, "turns": turn_list}
        return json.dumps(result)
...
class TweetSumProcessor:
    ...
    def get_dialog_with_summaries(
        self, tweet_sum_lines: List[str]
    ) -> List[DialogWithSummaries]:
        result = list()
        ...
        for tweet_sum_line in tweet_sum_lines:
            ...
            dialog_with_summaries = DialogWithSummaries(
